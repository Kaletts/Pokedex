...
//Selecciono la pokeball
let pokemonImage = document.getElementById('pokemonImage'); 
//Le agrego la clase par aanimarla
pokemonImage.classList.add('pokeBallAnimated'); 
...
//Todos los tipos de pokemons posibles para comparar con el valor recibido
const allPokemonTypes = ["bug", "dark", "dragon", "electric", "fairy", "fighting", "fire", "flying", "ghost", "grass", "ground", "ice", "normal", "poison", "psychic", "rock", "steel", "water"];
//Objeto con las traducciones de los pokemon pero donde la clave de esta en ingles, entonces es facil buscar.
const pokemonTypeSpanish = {
    bug: 'bicho',
    dark: 'siniestro',
    dragon: 'dragón',
    electric: 'eléctrico',
...
//Funcion principal que busca el pokemon y la descripción - tengo que refactorizar esto en funciones más pequeñas
function getPokemon(maxPokemonCount) {
    ...
    eliminarTodosLosTipos();//Elimino todos los tipos presentes anteriores
    ...
    fetch(URLApi)
        ...
        .then(data => {
            ...
            //Selecciono el elemento donde voy a completar el nombre
            let pokemonName = document.getElementById('pokemonName');
            //Selecciono el elemento donde voy a completar la imagen
            let pokemonImage = document.getElementById('pokemonImage');
            //Selecciono el div con clase pokemon-info para poder agregar justo luego el div nuevo con los SVGs;
            let infoDiv = document.querySelector('.pokemon-info');
            //Creo un div donde poner los SVG de tipos
            let divTypes = document.createElement('div');
            divTypes.style.display = 'flex';
            //Creamos un H2 para luego mostrar ahi el tipo de pokemon
            let typeOnScreen = document.createElement('h2');

            //Guardo el tipo de pokemon que se eligió
            let pokemonType = data.types[0].type.name;
            //Lo agrego al h2 que creamos arriba
            let typeSpanish = obtenerTipoEspañol(pokemonType);
            typeOnScreen.textContent = typeSpanish.charAt(0).toUpperCase() + typeSpanish.slice(1);

            //Creo un elemento img para guardar los svg
            const typeSvg = document.createElement('img');
            typeSvg.src = 'images/' + pokemonType + '.svg';
            typeSvg.classList.add('type-image');
            divTypes.classList.add('pokemon-types');

            //Agrega el div justo debajo de la descripción
            infoDiv.insertAdjacentElement('afterend', divTypes);
            divTypes.appendChild(typeSvg);
            divTypes.appendChild(typeOnScreen);

            //Paso la primera letra a mayuscula del nombre
            originalName = data.name;
            pokemonName.textContent = originalName.charAt(0).toUpperCase() + originalName.slice(1);

            //Guardo el ID global del pokemon para poder cambiar al anterior o al siguiente
            ...
            pokemonImage.classList.remove('pokeBallAnimated');//Quito la animación de la pokebola
            pokemonImage.classList.add('loaded');//Agrego el estilo de la imagen del pokemon
            ...
            arrayStats.forEach(stat => {
                document.getElementById('secondContainer').style.display = 'block';//Hago visible el contenedor
                ...
                bar.style.width = `${(statValue / 200) * 100}%`; // Ajusta la longitud de la barra según el nivel
...
function eliminarTodosLosTipos() {
    const elementosConClaseTipo = document.querySelectorAll('.pokemon-types');
    elementosConClaseTipo.forEach(elemento => {
        elemento.remove();
    });
}
...
//Funcion que devuelve el tipo en español comparando con el objeto global de tipos
function obtenerTipoEspañol(tipoIngles) {
    return pokemonTypeSpanish[tipoIngles] || tipoIngles;
}
